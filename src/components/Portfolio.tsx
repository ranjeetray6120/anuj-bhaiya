...
export default function Portfolio() {
  ...
  const projects = [
    {
      ...
      visual: "dashboard",
      color: "from-green-500/10 to-[#4eb46f]/20",
    },
    {
      ...
      visual: "wireframe",
      color: "from-[#4eb46f]/10 to-[#1e6ecc]/20",
    },
    {
      ...
      visual: "chart",
      color: "from-[#1e6ecc]/10 to-[#4eb46f]/20",
    },
    {
    ...
    },
    {
      ...
      visual: "wireframe",
      color: "from-green-500/10 to-teal-500/20",
    },
    {
  ...
  return (
    <section id="portfolio" className="py-20 bg-white border-t border-slate-200">
          ...
          <AnimatePresence mode="popLayout">
            {filteredProjects.map((project) => (
              <motion.div
                ...
                key={project.title}
                className="bg-white border border-slate-200 hover:border-slate-350 rounded-2xl overflow-hidden hover:shadow-lg transition-all group flex flex-col h-full"
              >
                {/* Visual Mock Container */}
                <div className={`h-48 relative overflow-hidden bg-gradient-to-br ${project.color} border-b border-slate-200 flex items-center justify-center p-6 transition-transform duration-500`}>
                  ...
                  {project.visual === "dashboard" && (
                    <div className="w-full h-full bg-slate-900 border border-slate-800 rounded-xl p-3 flex flex-col justify-between shadow-2xl relative text-left">
                      <div className="flex items-center justify-between border-b border-slate-800 pb-1.5">
                        ...
                        <span className="text-[8px] font-mono text-slate-500">Live API Data</span>
                      </div>
                      <div className="grid grid-cols-3 gap-2 py-2">
                        ...
                        <div className="bg-slate-950 p-1.5 rounded border border-slate-800/50 text-center">
                          ...
                          <span className="text-[10px] font-bold text-[#4eb46f]">894</span>
                        </div>
                        <div className="bg-slate-950 p-1.5 rounded border border-slate-800/50 text-center">
                          <span className="text-[7px] text-slate-500 block uppercase">ROAS</span>
                          <span className="text-[10px] font-bold text-[#1e6ecc]">4.5x</span>
                        </div>
                      </div>
                      <div className="w-full bg-slate-850 h-2 rounded overflow-hidden">
                        <div className="bg-[#4eb46f] h-full w-[70%]" />
                      </div>
                    </div>
                  )}
                  ...
                  {project.visual === "wireframe" && (
                    <div className="w-40 h-full bg-slate-900 border border-slate-800 rounded-t-xl p-3 flex flex-col gap-2 shadow-2xl relative top-2 text-left">
                      ...
                      <div className="w-full h-8 bg-gradient-to-r from-[#4eb46f]/20 to-[#1e6ecc]/20 rounded flex items-center justify-center">
                        <span className="text-[8px] font-bold text-[#4eb46f]">14.8% Conversion</span>
                      </div>
                    </div>
                  )}
                        ...
                        <svg className="w-full h-full" viewBox="0 0 100 40">
                          ...
                          <defs>
                            <linearGradient id="gradient-chart" x1="0%" y1="0%" x2="100%" y2="0%">
                              <stop offset="0%" stopColor="#4eb46f" />
                  ...
                  {project.visual === "video" && (
                    <div className="w-full h-full bg-slate-900 border border-slate-800 rounded-xl flex items-center justify-center shadow-2xl relative overflow-hidden group/vid text-left">
                      <div className="absolute inset-0 bg-slate-950/40 flex items-center justify-center">
                        <div className="w-10 h-10 rounded-full bg-white/10 border border-white/20 flex items-center justify-center group-hover/vid:scale-110 transition-transform">
                      ...
                      <div className="absolute bottom-2 left-2 right-2 flex justify-between items-center bg-slate-950/80 px-2 py-1 rounded border border-slate-800">
                        ...
                        <span className="text-[8px] font-mono text-[#4eb46f]">Scale Ready</span>
                      </div>
                    </div>
                  )}

                  {/* Absolute visual badge */}
                  <span className="absolute top-3 right-3 text-[10px] font-bold text-white bg-[#4eb46f] px-2.5 py-0.5 rounded-full z-20 shadow-sm">
                    {project.metric}
                  </span>
                </div>
                ...
                <div className="p-5 flex flex-col justify-between flex-1 space-y-4 text-left">
                  <div className="space-y-2">
                    <span className="text-[10px] font-bold text-[#1e6ecc] tracking-wider uppercase block">{project.meta}</span>
                    <h4 className="text-base font-bold text-slate-900 group-hover:text-[#4eb46f] transition-colors flex items-center gap-1.5 justify-between">
                      {project.title}
                      <ArrowUpRight className="w-4 h-4 opacity-0 group-hover:opacity-100 transition-opacity text-[#4eb46f]" />
                    </h4>
                  ...
                  <div className="pt-4 border-t border-slate-100 flex items-center justify-between text-xs font-semibold text-slate-450">
                    <span className="capitalize">Channel: {project.category === "ads" ? "Paid Ads" : project.category === "design" ? "Landing Pages" : "SEO Search"}</span>
                    <a href="#contact" className="hover:text-[#4eb46f] transition-colors flex items-center gap-0.5">
